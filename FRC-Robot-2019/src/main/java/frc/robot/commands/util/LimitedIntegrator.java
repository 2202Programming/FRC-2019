...
/** 
 *   LimitedIntegrator
 *      integrates input subject to rate limits
 * 
 *                    1
 *  in --> K -- RL --> -  ---> PL ---> out
 *                    s
 *   RL = rate limits,   PL = positiion limits
 * 
 * This is useful to ingetrate a joystick so you get 
 * a total position command.  That way when you let off 
 * the joystick, the position being commanded, X, stays.
 * 
...
public class LimitedIntegrator {
  ...
  public LimitedIntegrator(final Double dT,
      final DoubleSupplier inFunct,      //typical joystick function would go here
      double kGain,
      double x_min, 
      double x_max, 
      double dx_fall,  
