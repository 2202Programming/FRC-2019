...
public class SerialPortSubsystem extends Subsystem {
  ...
  public Integer getDistanceAvg(int sensor) { // return olympic average of last 10 readings of LIDAR sensor
    ...
    Integer[] tempArray = tempDeque.toArray(new Integer[0]); // switch from ArrayDeque to actual array so we can get
                                                             // each element
    Integer average = 0;
    Integer sum = 0;
    Integer max = 0;
    ...
    if (!serialExists)
      ...
      if ((distanceAvgArray.get(sensor - 1).size() - 2) != 0) {
        average = (sum - max - min) / (distanceAvgArray.get(sensor - 1).size() - 2); // return average, not including
                                                                                     // max or min reading (olympic)
        return average;
      } else
        return 0;
    }
  ...
  public Boolean isReliable(int sensor, double percent) { // check sensor to see if last 10 measurements are within a
                                                          // certain deviation
    Integer max = 0;
    Integer min = 10000;
    Deque<Integer> tempDeque = distanceAvgArray.get(sensor - 1);
  ...
  /*
   * public void serialReduction(int bufferLimit) { //throw away serial buffer
   * contents until size is < bufferLimit String temp;
   * 
   * while (arduinoSerial.getBytesReceived() > bufferLimit) { try { temp =
   * arduinoSerial.readString(1); //dpl was .readString(1); } catch
   * (UncleanStatusException e) { //Catch uncleanstatusexception and restart
   * serial port
   * System.out.println("Serial Exception UncleanStatusException caught. Code:" +
   * e.getStatus()); arduinoSerial.reset(); return; } } return; }
   */
  ...
  public void processMessage(StringBuilder serialResults) {
    ...
    distance = Integer.parseInt(serialResults.substring(2)); // read in everything after first two char as a distance
                                                             // (in mm)
